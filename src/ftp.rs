...
impl FtpStream {
    ...
    pub fn into_secure(mut self, ssl_context: SslContext) -> Result<FtpStream> {
        ...
        let ssl_cfg = try!(
            Ssl::new(&ssl_context).map_err(|e| FtpError::SecureError(e.description().to_owned()))
        );
        let stream = try!(
            ssl_cfg
    ...
    pub fn mdtm(&mut self, pathname: &str) -> Result<Option<DateTime<UTC>>> {
        ...
        match MDTM_RE.captures(&content) {
            Some(caps) => {
                ...
                Ok(Some(UTC.ymd(year, month, day).and_hms(
                    hour,
                    minute,
                    second,
                )))
    ...
    pub fn read_response_in(&mut self, expected_code: &[u32]) -> Result<Line> {
        ...
        let code: u32 = try!(line[0..3].parse().map_err(|err| FtpError::InvalidResponse(
